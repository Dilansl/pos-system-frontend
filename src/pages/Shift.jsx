      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200/60 p-6">
          <div className="flex items-center gap-2 mb-4">
            <span className="px-2 py-1 rounded text-xs bg-green-50 text-green-700 border border-green-200 flex items-center gap-1">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-slate-50 rounded p-3">
              ...
              <p className="text-lg font-bold text-slate-800">Rs. {Number(current.opening_cash).toLocaleString()}</p>
            </div>
            {current.notes && (
              <div className="bg-slate-50 rounded p-3">
          ...
          <h3 className="font-medium text-slate-700 mb-3">Close Shift</h3>
          <p className="text-sm text-slate-500 mb-3">
            Count the cash in the drawer and enter the total below.
          </p>
          <div className="mb-3">
            <label className="block text-xs text-slate-500 mb-1">Closing Cash (counted)</label>
            <input
              type="number"
              value={closingCash}
        ...
        <>
          {lastClosed && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-5 mb-6">
              <h3 className="font-bold text-slate-800 mb-3">Shift Closed — Summary</h3>
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div><span className="text-slate-500">Opening Cash:</span> Rs. {Number(lastClosed.opening_cash).toLocaleString()}</div>
                <div><span className="text-slate-500">Cash Sales:</span> Rs. {Number(lastClosed.cash_sales || 0).toLocaleString()}</div>
                <div><span className="text-slate-500">Expected in Drawer:</span> Rs. {expectedDrawer(lastClosed, lastClosed.cash_sales).toLocaleString()}</div>
                <div><span className="text-slate-500">Counted (Closing):</span> Rs. {Number(lastClosed.closing_cash).toLocaleString()}</div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200/60 p-6">
            <div className="flex items-center gap-2 mb-4">
              <span className="px-2 py-1 rounded text-xs bg-slate-100 text-slate-600 flex items-center gap-1">
                <FaLock size={10} /> No Open Shift
              </span>
            </div>
            <h3 className="font-medium text-slate-700 mb-3">Open Shift</h3>
            <p className="text-sm text-slate-500 mb-3">
              Enter the starting cash in the drawer to begin.
            </p>
            <div className="mb-3">
              <label className="block text-xs text-slate-500 mb-1">Opening Cash</label>
              <input
                type="number"
                value={openingCash}
